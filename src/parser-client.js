(() => {
  ...
  function snapshot(extra={}){ return {...telemetry,elapsedMs:Date.now()-new Date(telemetry.startedAt).getTime(),requestsLast60s:requestTimes.length,currentRun:window.EVERCADE_LOG?.currentRunSnapshot?.()||null,...extra}; }
  function waitWithSignal(ms,signal){ return new Promise((resolve,reject)=>{ if(signal?.aborted)return reject(new DOMException('Aborted','AbortError')); const timer=setTimeout(()=>{signal?.removeEventListener?.('abort',onAbort);resolve();},ms); const onAbort=()=>{clearTimeout(timer);signal?.removeEventListener?.('abort',onAbort);reject(new DOMException('Aborted','AbortError'));}; signal?.addEventListener?.('abort',onAbort,{once:true}); }); }
  ...
  async function probeWorkerHealth(reason='failure-streak') {
    ...
    telemetry.lastHealthProbeAt=new Date().toISOString(); telemetry.healthProbes+=1;
    healthProbePromise=(async()=>{ const started=performance.now(),url=`${config.genericParserWorkerUrl}/health?t=${Date.now()}`; log('info','worker.health.probe.start',{reason,url,telemetry:snapshot()}); try{ const response=await fetch(url,{cache:'no-store',mode:'cors',credentials:'omit'}); const text=await response.text(); let body=null; try{body=text?JSON.parse(text):{};}catch{body=text.slice(0,1000);} const result={reason,ok:response.ok,status:response.status,responseType:response.type,durationMs:Math.round(performance.now()-started),build:body?.build_id||body?.build||null,version:body?.version||null,cfRay:response.headers.get('cf-ray')||null,telemetry:snapshot()}; log(response.ok?'info':'warn','worker.health.probe.complete',result); return result;}catch(error){telemetry.healthProbeFailures+=1;const result={reason,ok:false,status:null,durationMs:Math.round(performance.now()-started),errorName:error?.name||null,message:error?.message||String(error),telemetry:snapshot()};log('error','worker.health.probe.failure',result);return result;}finally{healthProbePromise=null;} })();
  ...
  async function requestJson(url,{method='GET',body=null,signal,attempt=0,headers:extraHeaders={}}={}){
    ...
    try{
      const response=await fetch(url,options),durationMs=Math.round(performance.now()-started),text=await response.text(); let data; try{data=text?JSON.parse(text):{};}catch{throw new Error(`Ungültige JSON-Antwort (HTTP ${response.status})`);}
      const hitCount=collectListings(data).length,cfRay=response.headers.get('cf-ray')||null,workerRequestId=response.headers.get('x-request-id')||cfRay||null,workerVersion=response.headers.get('x-genericparser-version')||null,workerBuild=response.headers.get('x-genericparser-build')||null;
