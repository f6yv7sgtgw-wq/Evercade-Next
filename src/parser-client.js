(() => {
  ...
  const parseMoney = value => {
    ...
    if (typeof value === 'object') {
      for (const candidate of [value.value,value.amount,value.price,value.total,value.raw,value.text,value.formatted]) {
  ...
  function normalizeOffer(raw, fallbackSource = 'Kleinanzeigen') {
    ...
    const shipping = firstMoney([
      raw?.shipping,raw?.versand,raw?.shippingCost,raw?.shipping_cost,raw?.shippingText,raw?.shipping_text
    ],{allowZero:true});
    const explicitTotal = firstMoney([
      raw?.totalText,raw?.total_text,raw?.totalPrice,raw?.total_price,raw?.gesamtpreis,raw?.total
    ],{allowZero:false});
    const url = safeUrl(raw?.url ?? raw?.link ?? raw?.href);
    if (!url || price == null) {
      log('warn','offer.rejected.invalid_price',{
        source:raw?.source || fallbackSource,
        title:raw?.title || raw?.titel || null,
        rawPrice:raw?.price ?? null,
        priceText:raw?.priceText ?? raw?.price_text ?? raw?.displayPrice ?? null,
        url:url || null
      });
      return null;
    }
  ...
  function collectListings(payload) {
    const candidates = [
      payload?.listings,payload?.results,payload?.items,payload?.offers,
      payload?.data?.listings,payload?.data?.results,payload?.data?.items,payload?.data?.offers,
      payload?.response?.listings,payload?.response?.results,payload?.response?.items,payload?.response?.offers,
  ...
  async function requestJson(url, { method='GET', body=null, signal, attempt=0, headers:extraHeaders={} } = {}) {
    ...
    const isGenericParser = target.origin === new URL(config.genericParserWorkerUrl).origin;
    if (isGenericParser) {
      headers['x-generic-parser-contract'] = config.genericParserContract;
    ...
    try {
      ...
      if (!isAbort && isGenericParser && telemetry.consecutiveFailures >= 3) {
  ...
  async function searchLegacyRetailers(item, options = {}) {
    ...
    const statuses = Array.isArray(data.sources)
      ? data.sources.map(source=>({
          name:source.name || 'Händler',
          status:source.status || 'unknown',
          count:Number(source.accepted ?? source.count ?? 0) || 0,
  ...
  function directSearches(item) {
    const query=encodeURIComponent(`Evercade ${normalizeSearchTitle(item.title)}`);
  ...
  function dedupeOffers(offers) {
    ...
    for (const offer of offers) {
      ...
      try {
        ...
        url.hash='';
        key=`${String(offer.source).toLowerCase()}|${url.origin}${url.pathname}${url.search}`;
      } catch {
        key=`${String(offer.source).toLowerCase()}|${offer.id}`;
      }
      const previous=map.get(key);
      if (!previous || ((offer.total ?? Infinity) < (previous.total ?? Infinity))) map.set(key,offer);
    }
    return [...map.values()];
  }

  async function search(item, options = {}) {
    const [kleinanzeigen,retailers] = await Promise.allSettled([
      searchKleinanzeigen(item,options),
      searchLegacyRetailers(item,options)
    ]);

    if (kleinanzeigen.status==='rejected' && kleinanzeigen.reason?.name==='AbortError') throw kleinanzeigen.reason;
    if (retailers.status==='rejected' && retailers.reason?.name==='AbortError') throw retailers.reason;

    const offers=[];
    const status=[];

    if (kleinanzeigen.status==='fulfilled') {
      offers.push(...kleinanzeigen.value.offers);
      status.push({name:'Kleinanzeigen',status:'ok',count:kleinanzeigen.value.offers.length,endpoint:kleinanzeigen.value.endpoint});
    } else {
      status.push({name:'Kleinanzeigen',status:'error',count:0,error:kleinanzeigen.reason?.message || String(kleinanzeigen.reason)});
    }

    if (retailers.status==='fulfilled') {
      offers.push(...retailers.value.offers);
      if (retailers.value.statuses.length) status.push(...retailers.value.statuses);
      else status.push({name:'9 Händler',status:'ok',count:retailers.value.offers.length});
    } else {
      status.push({name:'9 Händler',status:'error',count:0,error:retailers.reason?.message || String(retailers.reason)});
    }

    if (kleinanzeigen.status==='rejected' && retailers.status==='rejected') {
      throw new Error(`Keine automatische Quelle erreichbar (Kleinanzeigen: ${kleinanzeigen.reason?.message}; Händler: ${retailers.reason?.message})`);
    }
  ...
  window.EvercadeSearch=Object.freeze({search,searchKleinanzeigen,searchLegacyRetailers,directSearches,normalizeOffer,normalizeSearchTitle,requestJson,collectListings,telemetry:snapshot});
})();
